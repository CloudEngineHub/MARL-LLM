...
from algorithm.utils.buffer_episode import ReplayBuffer as ReplayBufferRew
...
def run(cfg):
    ...
    reward_buffer = ReplayBufferRew(150)
    ...
    for ep_index in range(0, cfg.n_episodes, cfg.n_rollout_threads):
        ...
        for et_index in range(cfg.episode_length):
            ...
            state_rew.append(obs)
            act_rew.append(agent_actions)
            rew_rew.append(rewards)
            state_act_rew.append(np.vstack((obs, agent_actions)))
            ...
            # agent_buffer[1].push(obs_rs, agent_rewards_rs, rewards, next_obs_rs, dones, start_stop_num[0])
            #####--------------------------#####
        ...
        reward_buffer.append(state_rew, act_rew, rew_rew, state_act_rew, env.n_a, cfg.episode_length)
        ...
        for _ in range(3):
            for a_i in range(maddpg.nagents):
                traj_sample = reward_buffer.sample(5, to_gpu=True if cfg.device == 'gpu' else False)  
                maddpg.update_rew2(traj_sample, a_i, logger=logger)     # parameter update
