...
class FormationSwarmEnv(gym.Env):
    ...
    def __reinit__(self, args):
        ...
        self.num_neigh_grid = np.zeros(self.n_a, dtype=np.float64)
        self.num_neigh_grid_c = np.zeros(self.n_a, dtype=np.float64)
        ...
        if self.video:
            self.video = VideoWriter(output_rate=self.dt, fps=30)
    ...
    def reset(self):
        ...
        rand_target_offset = np.random.uniform(-1.4, 1.4, (2, 1))   ################## domain generalization 4
    ...
    def _get_obs(self):
        ...
        #                       as_double_c_array(self.num_neigh_grid_c),
        ...
        for i in range(self.n_a):
            ...
            if num_sensed_grid > self.num_obs_grid_max:
                ...
                self.num_neigh_grid[i] = num_sensed_grid / self.num_obs_grid_max
            else:
                sensed_grid_pos = None
                self.num_neigh_grid[i] = 0

            num_grid_rel = np.zeros((1, self.topo_nei_max))
            num_grid_rel[:,:nei_num] = self.num_neigh_grid[neigh_index] - self.num_neigh_grid[i]
            if self.is_con_self_state:
                obs_agent_pos = np.concatenate((self.p[:, [i]], relPos_a2a), axis=1)
                obs_agent_vel = np.concatenate((self.dp[:, [i]], relVel_a2a), axis=1)
                obs_agent_num_grid_rel = np.concatenate((self.num_neigh_grid[[[i]]], num_grid_rel), axis=1)
                obs_agent = np.concatenate((obs_agent_pos, obs_agent_vel, obs_agent_num_grid_rel), axis=0)
            else:
                obs_agent = np.concatenate((relPos_a2a, relVel_a2a, num_grid_rel), axis=0)
        ...
        ss = np.sum(bb - self.num_neigh_grid)
        if ss != 0:
            print(ss)
    ...
    def _get_reward(self, a):
        ...
        coefficients = np.array([5] + [10, 1] + [7, 3] + [1, 1, 1] + [5] + [12], dtype=np.float64) # reward #2 dense 5 10 1 7
        ...
        _LIB._get_reward(as_double_c_array(self.p), 
                         ...
                         as_double_c_array(self.boundary_pos),
                         as_double_c_array(self.grid_center),
                         as_double_c_array(self.num_neigh_grid),
                         as_int32_c_array(self.neighbor_index),
                         as_int32_c_array(self.in_flags),
                         as_int32_c_array(self.sensed_index),
                         as_int32_c_array(self.occupied_index),
    ...
    def render(self, mode="human"): 
        ...
        if self.plot_initialized == 0:
            ...
            #     circle = plt.Circle(self.p[:,iii], self.r_avoid/2, color='green', fill=False)
            #     ax.add_patch(circle)
