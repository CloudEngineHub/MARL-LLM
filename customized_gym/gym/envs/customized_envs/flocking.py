...
class FlockingSwarmEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Observation 
        self.topo_nei_max = 3   # agent to agent 
    ...
    def reset(self):
        self.simulation_time = 0
        # self.d_sen = np.random.randint(1, 5)
        self.d_sen = 6
        ...
        random_int = 3
        if self.is_leader:
            self.p = np.concatenate((np.random.uniform(-random_int, random_int, (2, self.n_l))/2, 
                                     np.random.uniform(-random_int, random_int, (2, self.n_ao - self.n_l))), axis=1)   # Initialize self.p
            # self.p = np.concatenate((np.random.uniform(-1, 1, (1, self.n_a)),
            #                          np.random.uniform(-1, 1, (1, self.n_a))), axis=0) + np.array([[2, -1.5]]).T
            # self.p[:,0] = np.zeros(2)
        else:
        ...
        if self.dynamics_mode == 'Cartesian':
            ...
            # random_integer = np.random.randint(0, 50) * 2 *np.pi / 49
            # # random_integer = 9
            # self.dp = np.tile([[np.cos(random_integer)], [np.sin(random_integer)]], (1, self.n_a))
        else:
            self.dp = np.zeros((self.dim, self.n_ao))
        ...
        if self.is_boundary:
            ...
            # self.p += self.leader_waypoint[:,[0]]
        else:
            self.leader_state[:self.dim] = np.zeros((2, self.n_l))
            # random_integer = np.random.randint(0, 16, size=self.n_l) * 2 *np.pi / 15
    ...
    def _get_obs(self):
        ...
        #         # leader_pos_rel_mat = self.p[:, :self.n_l] - self.p[:,[i]]
        #         if self.is_periodic:
    ...
    def _get_reward(self, a):
        ...
        # remarkable leader position, leader velocity
        # inter-agent collision
        # control effert
        # agent-obstacle collision
        # agent-wall collision
        # is_leader = false: {is_periodic = true, Polar: 15, 0.7, 2, Cartesian: 15, 0.8, 1.6.  is_boundary = true, Polar: 18, 0.8, 7 + 20  Cartesian: 18, 0.7, 7 + 12}
        # is_leader = true: {is_periodic = true, Polar: [24, 0.4, 1.6] + [4, 6], Cartesian: [24, 0.4, 1.6] + [1.4, 6].  is_boundary = true, Polar: 18, 0.8, 7 + 20  Cartesian: [24, 0.4, 1.6 + 1.4, 4]}. local leader + boundary: [24, 0.7, 2] + [4, 6]
        ...
        #         list_nei = self.neighbor_index[agent]
        ...
        #                 pos_relative = self.p[:,agent2] - self.p[:,agent]
        #                 if self.is_periodic:
        #                     pos_relative = self._make_periodic(pos_relative, is_rel=True)

        #                 dist_b2b = np.linalg.norm(pos_relative)
        #                 reward_a[0, agent] -= np.max([coefficients[0] * (self.d_ref - dist_b2b - 0.05), coefficients[1] * (dist_b2b - self.d_ref - 0.05)])
                    
        #             # alignment
        #             avg_neigh_vel = np.mean(self.dp[:,list_nei]/ (np.linalg.norm(self.dp[:,list_nei], axis=0) + 1e-8), axis=1)
        #             reward_a[0,agent] -= coefficients[2] * np.linalg.norm(avg_neigh_vel - self.dp[:,agent]/(np.linalg.norm(self.dp[:,agent]) + 1e-8))
        
        # # tracking virtual leader
        # if self.is_remarkable_leader:
        #     for agent in range(self.n_a):
        #         nearest_leader_index_ = self.nearest_leader_index[agent]
        #         if nearest_leader_index_ >= 0:
        #             # position
        #             leader_agent_pos_rel = self.leader_state[:self.dim, nearest_leader_index_] - self.p[:, agent]
        #             # leader_agent_pos_rel = self.p[:, nearest_leader_index_] - self.p[:, agent]
        #             if self.is_periodic:
        #                 leader_agent_pos_rel = self._make_periodic(leader_agent_pos_rel, is_rel=True)
        #             reward_a[0,agent] -= coefficients[3] * np.linalg.norm(leader_agent_pos_rel) # 4.5

        #             # velocity  
        #             reward_a[0,agent] -= coefficients[4] * np.linalg.norm(self.leader_state[self.dim:2*self.dim, nearest_leader_index_] - self.dp[:, agent])
      
        # # collision avoidance
        # if self.penalize_collide_agents:
        #     reward_a -= coefficients[5] * self.is_collide_b2b[:self.n_a, :self.n_a].sum(axis=0, keepdims=True)
        ...
        #     reward_a -= coefficients[9] * self.is_collide_b2b[self.n_a:self.n_ao, :self.n_a].sum(axis=0, keepdims=True)          
