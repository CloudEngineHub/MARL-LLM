                      ...
                      bool *condition) 
{
    ...
    Matrix matrix_dp(dim, std::vector<double>(n_pe));
    Matrix matrix_heading(dim, std::vector<double>(n_pe));
    Matrix matrix_obs(obs_dim_agent, std::vector<double>(n_pe));
    ...
    for (int i = 0; i < dim; ++i) {
        for (int j = 0; j < n_pe; ++j) {
            ...
            matrix_dp[i][j] = dp_input[i * n_pe + j];
            matrix_heading[i][j] = heading_input[i * n_pe + j];
        }
    }
    ...
    for (int agent_i = 0; agent_i < n_p; ++agent_i) {
        //////////////////////////////// predator-predator ////////////////////////////////
        Matrix relPos_p2p(dim, std::vector<double>(n_p, 0.0));
        Matrix relVel_p2p(dim, std::vector<double>(n_p, 0.0));
        for (int j = 0; j < n_p; ++j) {
            for (int k = 0; k < dim; ++k) {
                relPos_p2p[k][j] = matrix_p[k][j] - matrix_p[k][agent_i];
                if (condition[1]) {
                    relVel_p2p[k][j] = matrix_dp[k][j] - matrix_dp[k][agent_i];
                } else {
                    relVel_p2p[k][j] = matrix_heading[k][j] - matrix_heading[k][agent_i];
                }
            }
        }
        if (condition[0]) {
            _make_periodic(relPos_p2p, L, boundary_pos, true);
        }
        // Obtain focused observations
        std::tuple<Matrix, Matrix, std::vector<int>> focused_obs_p2p = _get_focused(relPos_p2p, relVel_p2p, d_sen_p, topo_nei_p2p, true);
        Matrix relPos_p2p_focused = std::get<0>(focused_obs_p2p);
        Matrix relVel_p2p_focused = std::get<1>(focused_obs_p2p);

        //////////////////////////////// predator-prey ////////////////////////////////
        Matrix relPos_p2e(dim, std::vector<double>(n_e, 0.0));
        Matrix relVel_p2e(dim, std::vector<double>(n_e, 0.0));
        for (int j = 0; j < n_e; ++j) {
            for (int k = 0; k < dim; ++k) {
                relPos_p2e[k][j] = matrix_p[k][j + n_p] - matrix_p[k][agent_i];
                if (condition[1]) {
                    relVel_p2e[k][j] = matrix_dp[k][j + n_p] - matrix_dp[k][agent_i];
                } else {
                    relVel_p2e[k][j] = matrix_heading[k][j + n_p] - matrix_heading[k][agent_i];
                }
            }
        }
        if (condition[0]) {
            _make_periodic(relPos_p2e, L, boundary_pos, true);
        }
        // Obtain focused observations
        std::tuple<Matrix, Matrix, std::vector<int>> focused_obs_p2e = _get_focused(relPos_p2e, relVel_p2e, d_sen_p, topo_nei_p2e, false);
        Matrix relPos_p2e_focused = std::get<0>(focused_obs_p2e);
        Matrix relVel_p2e_focused = std::get<1>(focused_obs_p2e);
        ...
        if (condition[2]) { // whether contain myself state in the observation
            ...
            // }
            Matrix obs_pursuer_pos = _concatenate(_extract_column(matrix_p, agent_i), relPos_p2p_focused, 1);
            Matrix obs_pursuer_vel = _concatenate(_extract_column(matrix_dp, agent_i), relVel_p2p_focused, 1);
            // Matrix obs_pursuer_vel = _concatenate(vel_or_heading_p, relVel_p2p_focused, 1);
            Matrix obs_pursuer_pos_2 = _concatenate(obs_pursuer_pos, relPos_p2e_focused, 1);
            Matrix obs_pursuer_vel_2 = _concatenate(obs_pursuer_vel, relVel_p2e_focused, 1);
            obs_agent = _concatenate(obs_pursuer_pos_2, obs_pursuer_vel_2, 0);
        } else {
            Matrix obs_pursuer_pos_2 = _concatenate(relPos_p2p_focused, relPos_p2e_focused, 1);
            Matrix obs_pursuer_vel_2 = _concatenate(relVel_p2p_focused, relVel_p2e_focused, 1);
            obs_agent = _concatenate(obs_pursuer_pos_2, obs_pursuer_vel_2, 0);
        }
    ...
    for (int agent_i = n_p; agent_i < n_pe; ++agent_i) {
        //////////////////////////////// escaper-predator ////////////////////////////////
        Matrix relPos_e2p(dim, std::vector<double>(n_p, 0.0));
        Matrix relVel_e2p(dim, std::vector<double>(n_p, 0.0));
        for (int j = 0; j < n_p; ++j) {
            for (int k = 0; k < dim; ++k) {
                relPos_e2p[k][j] = matrix_p[k][j] - matrix_p[k][agent_i];
                if (condition[1]) {
                    relVel_e2p[k][j] = matrix_dp[k][j] - matrix_dp[k][agent_i];
                } else {
                    relVel_e2p[k][j] = matrix_heading[k][j] - matrix_heading[k][agent_i];
                }
            }
        }
        if (condition[0]) {
            _make_periodic(relPos_e2p, L, boundary_pos, true);
        }
        // Obtain focused observations
        std::tuple<Matrix, Matrix, std::vector<int>> focused_obs_e2p = _get_focused(relPos_e2p, relVel_e2p, d_sen_e, topo_nei_e2p, false);
        Matrix relPos_e2p_focused = std::get<0>(focused_obs_e2p);
        Matrix relVel_e2p_focused = std::get<1>(focused_obs_e2p);

        //////////////////////////////// predator-prey ////////////////////////////////
        Matrix relPos_e2e(dim, std::vector<double>(n_e, 0.0));
        Matrix relVel_e2e(dim, std::vector<double>(n_e, 0.0));
        for (int j = 0; j < n_e; ++j) {
            for (int k = 0; k < dim; ++k) {
                relPos_e2e[k][j] = matrix_p[k][j + n_p] - matrix_p[k][agent_i];
                if (condition[1]) {
                    relVel_e2e[k][j] = matrix_dp[k][j + n_p] - matrix_dp[k][agent_i];
                } else {
                    relVel_e2e[k][j] = matrix_heading[k][j + n_p] - matrix_heading[k][agent_i];
                }
            }
        }
        if (condition[0]) {
            _make_periodic(relPos_e2e, L, boundary_pos, true);
        }
        // Obtain focused observations
        std::tuple<Matrix, Matrix, std::vector<int>> focused_obs_e2e = _get_focused(relPos_e2e, relVel_e2e, d_sen_e, topo_nei_e2e, true);
        Matrix relPos_e2e_focused = std::get<0>(focused_obs_e2e);
        Matrix relVel_e2e_focused = std::get<1>(focused_obs_e2e);
        ...
        if (condition[2]) { // whether contain myself state in the observation
            ...
            // }
            Matrix obs_escaper_pos = _concatenate(_extract_column(matrix_p, agent_i), relPos_e2p_focused, 1);
            Matrix obs_escaper_vel = _concatenate(_extract_column(matrix_dp, agent_i), relVel_e2p_focused, 1);
            // Matrix obs_escaper_vel = _concatenate(vel_or_heading_e, relVel_e2p_focused, 1);
            Matrix obs_escaper_pos_2 = _concatenate(obs_escaper_pos, relPos_e2e_focused, 1);
            Matrix obs_escaper_vel_2 = _concatenate(obs_escaper_vel, relVel_e2e_focused, 1);
            obs_agent = _concatenate(obs_escaper_pos_2, obs_escaper_vel_2, 0);
        } else {
            Matrix obs_escaper_pos_2 = _concatenate(relPos_e2p_focused, relPos_e2e_focused, 1);
            Matrix obs_escaper_vel_2 = _concatenate(relVel_e2p_focused, relVel_e2e_focused, 1);
            obs_agent = _concatenate(obs_escaper_pos_2, obs_escaper_vel_2, 0);
        }
    ...
    for (int i = 0; i < obs_dim_agent; ++i) {
        for (int j = 0; j < n_pe; ++j) {
            obs_input[i * n_pe + j] = matrix_obs[i][j];
        }
    }
...
{
    Matrix matrix_p(dim, std::vector<double>(n_pe));
    Matrix matrix_d_b2b_edge(n_pe, std::vector<double>(n_pe));
    Matrix matrix_d_b2b_center(n_pe, std::vector<double>(n_pe));
    std::vector<std::vector<bool>> is_collide_b2b(n_pe, std::vector<bool>(n_pe, false));
    std::vector<double> boundary_pos(4, 0.0);
    ...
    for (int i = 0; i < n_pe; ++i) {
        for (int j = 0; j < n_pe; ++j) {
            matrix_d_b2b_edge[i][j] = d_b2b_edge_input[i * n_pe + j];
            matrix_d_b2b_center[i][j] = d_b2b_center_input[i * n_pe + j];
            is_collide_b2b[i][j] = is_collide_b2b_input[i * n_pe + j];
        }
    }
    ...
    Matrix sf_b2b_all(2 * n_pe, std::vector<double>(n_pe, 0.0));
    double L = (boundary_pos[2] - boundary_pos[0]) / 2.0;
    ...
    // 计算 sf_b2b
    Matrix sf_b2b(2, std::vector<double>(n_pe));
    for (int i = 0; i < n_pe; ++i) {
        for (int j = 0; j < dim; ++j) {
            double sum = 0.0;
    ...
    for (int i = 0; i < dim; ++i) {
        for (int j = 0; j < n_pe; ++j) {
           sf_b2b_input[i * n_pe + j] = sf_b2b[i][j];
        }
    }
