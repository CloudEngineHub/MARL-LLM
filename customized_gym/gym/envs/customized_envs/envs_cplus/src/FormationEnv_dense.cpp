                      ...
                      bool *condition) 
{
    ...
    for (int agent_i = 0; agent_i < n_a; ++agent_i) {
        ...
        if (num_sensed_grid_origin > 0) {
            ...
            if (in_flags[agent_i] == 1) {
                ...
                for (size_t j = 0; j < agent_pos_rel_norm.size(); ++j) {
                    if (j != agent_i && agent_pos_rel_norm[j] < (d_sen + r_avoid/2)) {
                ...
                if (!nearby_agents.empty()) {
                    for (int nearby_i : nearby_agents) {
                        ...
                        for (size_t j = 0; j < grid_neigh_pos_relative_norm.size(); ++j) {
                            mask[j] = grid_neigh_pos_relative_norm[j] > r_avoid/2;
                        }
        ...
        if (sensed_indices.size() > num_obs_grid_max) {
            Matrix sensed_grid_pos_1(2, std::vector<double>(num_obs_grid_max));
            // std::cout << 11111 << std::endl;
            // 计算步长
            double step = static_cast<double>(sensed_indices.size() - 1) / (num_obs_grid_max - 1); // -1 是为了确保第一个和最后一个元素都选到
        ...
        } else if (sensed_indices.size() > 0 && sensed_indices.size() <= num_obs_grid_max) {
            Matrix sensed_grid_pos_2(2, std::vector<double>(sensed_indices.size()));
            for (size_t j = 0; j < sensed_indices.size(); ++j) {
            ...
            for (int i = 0; i < sensed_indices.size(); ++i) {
        ...
        for (int agent_i = 0; agent_i < n_a; ++agent_i) {
            ...
            if (!list_grid.empty()) {
                ...
                if (!sensed_grid[0].empty()) {
                    ...
                    // std::cout << "agent_i: " << agent_i << "v_exp_i: " << v_exp_i[0] << v_exp_i[1] << std::endl;

                    // strategy 1
                    // double v_exp_i_norm = _norm(v_exp_i) + 1E-8;
                    // std::vector<double> dp_agent = _extract_column_one(matrix_dp, agent_i);
                    // double norm_dp_agent = _norm(dp_agent) + 1E-8;
                    // reward_a[agent_i] -= coefficients[3] * std::sqrt(std::pow(v_exp_i[0]/v_exp_i_norm - matrix_dp[0][agent_i]/norm_dp_agent, 2) + 
                    //                                                  std::pow(v_exp_i[1]/v_exp_i_norm - matrix_dp[1][agent_i]/norm_dp_agent, 2));

                    // strategy 2
                    reward_a[agent_i] -= coefficients[3] * std::sqrt(std::pow(v_exp_i[0], 2) + std::pow(v_exp_i[1], 2));
                }
            }
        }
    }
