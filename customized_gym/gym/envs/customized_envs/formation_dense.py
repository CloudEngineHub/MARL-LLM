...
class FormationSwarmEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        #                     mask = grid_neigh_pos_relative_norm > self.r_avoid/2
    ...
    def _get_reward(self, a):
        ...
        coefficients = np.array([5] + [10, 0] + [10] + [1, 1, 1] + [5] + [12], dtype=np.float64)
        ...
        #                 v_exp_i = 3 * numerator / denominator  # shape: [dim] expected exploration velocity
        #                 # print("python_v_exp_i:", v_exp_i)

        #                 # strategy 1
        #                 v_exp_i_norm = np.linalg.norm(v_exp_i) + 1e-8
        #                 v_agent_i_norm = np.linalg.norm(self.dp[:,agent_i]) + 1e-8
        #                 reward_a[0,agent_i] -= coefficients[3] * np.linalg.norm(v_exp_i/v_exp_i_norm - self.dp[:,agent_i]/v_agent_i_norm)

        #                 # strategy 2
        #                 # reward_a[0,agent_i] -= coefficients[3] * np.linalg.norm(v_exp_i)
