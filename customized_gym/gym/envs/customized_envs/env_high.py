...
class EnvHighSwarmEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.topo_nei_max = 2   # agent to agent
        ...
        self.linVel_l_max = 0.3  # 0.8
        self.linVel_r_max = 0.3  # 0.8
        ...
        self.way_count_1 = 1
        self.way_count_2 = 1
        self.leader_v = 0.2 # 0.7

        ## ======================================== end ========================================

        # Half boundary length
        self.boundary_L_half = 2.5 # 6
    ...
    def reset(self):
        # initial variables
        self.simulation_time = 0
        # self.d_sen = np.random.randint(1, 3)
        ...
        W_group = 1 # 1.7
        L_group = 1 # 2
        # W_group = np.random.uniform(0.5,2)
        # L_group = np.random.uniform(0.5,2)
        ...
        p_l = np.concatenate((np.random.uniform(-L_group, L_group, (1, self.n_l)),
                              np.random.uniform(-W_group, W_group, (1, self.n_l))), axis=0) + np.array([[2, -1.5]]).T
        p_r = np.concatenate((np.random.uniform(-L_group, L_group, (1, self.n_r)),
                              np.random.uniform(-W_group, W_group, (1, self.n_r))), axis=0) + np.array([[-2, 1.5]]).T
        # xdd = np.random.uniform(-3,3)
        # ydd = np.random.uniform(-3,3)
        # p_l = np.concatenate((np.random.uniform(-L_group, L_group, (1, self.n_l)),
        #                       np.random.uniform(-W_group, W_group, (1, self.n_l))), axis=0) + np.array([[xdd, -ydd]]).T
        # p_r = np.concatenate((np.random.uniform(-L_group, L_group, (1, self.n_r)),
        #                       np.random.uniform(-W_group, W_group, (1, self.n_r))), axis=0) + np.array([[-xdd, ydd]]).T
        ...
        # center_l = np.mean(p_l, axis=1)
        # center_r = np.mean(p_r, axis=1)
        # distance = np.linalg.norm(center_l - center_r)
        # unit_direction = (center_r - center_l) / distance
        # p1_extended = center_l - 7 * unit_direction
        # p2_extended = center_r + 7 * unit_direction
        # new_distance = np.linalg.norm(p2_extended - p1_extended)
        # num_points = int(new_distance / 0.1) + 1
        # x_c = np.linspace(p1_extended[0], p2_extended[0], num_points)
        # y_c = np.linspace(p1_extended[1], p2_extended[1], num_points)
        # line_coordinates = np.vstack((x_c, y_c))
        # self.leader_waypoint_1 = line_coordinates.copy()
        # self.leader_waypoint_2 = line_coordinates[:, ::-1].copy()
        ...
        #                               [-offset, 0]]).T
        # leader_pos_offset = np.array([[0, offset], 
        #                                    [0.866 * offset, -0.5 * offset], 
        #                                    [-0.866 * offset, -0.5 * offset]]).T
        leader_pos_offset = np.array([[0, 0]]).T
        ...
        self.random_leader_offset = np.random.uniform(-0.0, 0.0, (self.dim, 2*self.n_leader))
    ...
    def _get_obs_flocking(self, group_index):
        ...
        # bb = self.obs
        # self.obs = np.zeros((self.observation_space_flocking.shape[0], num_agents))

        # for i in range(num_agents):
        #     '''
        #     The agent's observation, including the state of the neighbors and virtual leader
        #     '''
        #     relPos_a2a = p - p[:,[i]]
        #     if self.is_periodic:
        #         relPos_a2a = self._make_periodic(relPos_a2a, is_rel=True)
        #     relVel_a2a = dp - dp[:,[i]] if self.dynamics_mode == 'Cartesian' else heading - heading[:, [i]]
        ...
        #         obs_agent_pos = np.concatenate((p[:, [i]], relPos_a2a), axis=1)
        #         obs_agent_vel = np.concatenate((dp[:, [i]], relVel_a2a), axis=1)
        #         obs_agent = np.concatenate((obs_agent_pos, obs_agent_vel), axis=0)  # (4, n_peo+1) FIXME: only suitable for no obstacles
        #     else:
        #         obs_agent = np.concatenate((relPos_a2a, relVel_a2a), axis=0)

        #     if self.is_leader and self.is_remarkable_leader:
        #         leader_pos_rel_mat = leader_state[:self.dim, :] - p[:,[i]]
        #         if self.is_periodic:
        ...
        #             leader_vel_rel = leader_state[self.dim:, min_index] - dp[:,i]
        #             norm_1 = np.linalg.norm(leader_state[self.dim:, min_index]) + 1e-8
        #             leader_heading_rel = leader_state[self.dim:, min_index] / norm_1 - heading[:,i]
        ...
        #             self.obs[self.obs_dim_agent - self.dim:, i] = heading[:,i]
        #     else:
        #         if self.dynamics_mode == 'Cartesian':
    ...
    def get_observation_rl(self):
        ...
        # obs_l_state_l = np.append(self.center_leader_state[:,0], self.n_l)
        # obs_l_state_r = np.zeros_like(obs_l_state_l)
        # obs_r_state_r = np.append(self.center_leader_state[:,1], self.n_r)
        # obs_r_state_l = np.zeros_like(obs_r_state_r)
        # dist_center_leader = np.linalg.norm(self.center_leader_state[:self.dim,0] - self.center_leader_state[:self.dim,1])
        # if dist_center_leader < self.d_sen:
        #     obs_l_state_r = obs_r_state_r
        #     obs_r_state_l = obs_l_state_l

        # obs[:,0] = np.concatenate((obs_l_state_l, obs_l_state_r))
        # obs[:,1] = np.concatenate((obs_r_state_r, obs_r_state_l))
        ...
        # dist_center_leader = np.linalg.norm(self.center_leader_state[:self.dim,0] - self.center_leader_state[:self.dim,1])
        # if dist_center_leader < 2 * self.d_sen:
    ...
    def _judge_stage(self):
        if self.stage_index == 0:
            if self.n_l > 0 and self.n_r > 0:
                ...
                #     rel_p = self.p[:, self.index_r] - self.p[:, [self.index_l[i]]]
                #     rel_p_norm = np.linalg.norm(rel_p, axis=0)
                #     dist_min = np.min(rel_p_norm)
                #     if dist_min < self.d_sen:
                #         self.stage_index = 1
                #         break
                rel_p = self.p[:,self.index_l] - self.center_leader_state[:self.dim,[1]]
                rel_p_norm = np.linalg.norm(rel_p, axis=0)
            ...
            def apply_strategy(strategy, is_left, a, n_last, n_lr_last):
                ...
                if strategy == 'input':
                    ...
                    if self.n_l_last > 0 and self.n_r_last > 0:
                        ...
                        goto_dir = 1.5 * delta_p + 1 * delta_v # 1, 2
                        if self.dynamics_mode == 'Cartesian':
                            a_rec = goto_dir
                        elif self.dynamics_mode == 'Polar':
